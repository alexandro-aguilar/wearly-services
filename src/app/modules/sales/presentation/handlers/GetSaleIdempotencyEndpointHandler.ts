...
import { IdempotencyStatusDto } from '@src/app/modules/sales/application/dtos/CheckoutDto';
import container from '@src/app/modules/sales/config/container';
import types from '@src/app/modules/sales/config/types';
const schema: ValidatorSchemas = { pathParameters: Joi.object({ key: Joi.string().required() }).required() };
...
export const handler = middy(
  async (event: APIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<IdempotencyStatusDto>> => {
    ...
    return container
      ...
      .status(principal, event.pathParameters?.key ?? '');
  }
)
