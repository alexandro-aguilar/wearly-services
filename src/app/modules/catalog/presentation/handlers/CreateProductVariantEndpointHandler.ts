...
import { CreateProductVariantHandler } from '../../application/commands/CreateProductVariantHandler';
import Joi from '@hapi/joi';
import Authorization from '@src/app/core/interface/Authorization';

const createProductVariantEndpointHandlerSchema: ValidatorSchemas = {
  body: Joi.object({
...
export const handler = middy(
  async (event: APIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<{ id: string }>> => {
    ...
    const createProductVariantCommand = JSON.parse(event.body || '{}');
    const authenticatedPrincipal = await Authorization.authenticate(event.headers);
    const createProductVariantHandler = container.get<CreateProductVariantHandler>(types.CreateProductVariantHandler);
    const response = await createProductVariantHandler.execute(authenticatedPrincipal, createProductVariantCommand);

    return response;
  }
)
  .use(requestValidator(createProductVariantEndpointHandlerSchema))
